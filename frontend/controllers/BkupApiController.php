            ...
            $accessStatus = Yii::$app->utility->authenticateAccessToken($token);
                if ($accessStatus['authentication'] === 1){
                         ...
                         foreach($GroupMessageModel as $gp_me){
                             ...
                             $Report['sender_image']=$UserModel->image_path.$UserModel->image_name;*/
    ...
    public function actionGetTrainingSet(){
        if (Yii::$app->request->post()) {
            ...
            if ($accessStatus['authentication'] === 1) {
                ...
                $getTrainingStratDate = $trainingModel->start_date;
                $getTrainingEndDate = $trainingModel->end_date;
                $getCurrentTime = date("Y-m-d H:i:s");
                if($getCurrentTime>=$getTrainingStratDate){
                    ...
                    if(empty($models)>0){
                        ...
                        foreach ($models as $key => $model){
                            ...
                            if(empty($optionModel)){
                                ...
                                $response = [
                                    'status' => 1,
                                    'message' => 'Options are not available this question',
                                    'data' => [],
                                ];
                            } else {
                                ...
                                $data[] = [
                                    ...
                                    "correct_answer" => $optionModel->id,
                                    "negative_mark" => $model->negative_mark,
                                    "positive_marks" => $model->marks,
                                    "answer_given" => empty($submissionModel) === false ?$submissionModel->option_id:"",
                                    "options" => $options
                                ];
                    ...
                    $response = [
                        'status' => 0,
                        'message' => 'Test will start on '. date("h:i A",strtotime($trainingModel->start_date))
                    ];
    ...
    public function actionSubmitAssessment(){
        if (Yii::$app->request->post()) {
            $postArr = [
                ...
                "oid" => isset($_POST['oid']) && !empty($_POST['oid'])?$_POST['oid']:null,
...
//                $Report = array();
    ...
    public function actionAppLogin() {
       ...
       if (Yii::$app->request->post()) {
            ...
            if ($model->login()) {
                if(Yii::$app->user->identity->form_submit!=1){
                     $response = [
                        'status' => 2,
                        'id'=>Yii::$app->user->identity->id,
                        'message' => 'You have not yet completed the registration process !!'
                    ];
                }else
            ...
            if ($accessStatus['authentication'] === 1) {
                ...
                if(empty($model)){
                    ...
                    $response = [
                        ...
                        'webcast_link' => 'https://www.youtube.com/embed/'.$model->url.'?autoplay=1&vq=large&refl=2&wmode=opaque&amp;rel=0&amp;autohide=1&amp;showinfo=0&amp;wmode=transparent',
                    ];
                }
            } else {
                $response = [
