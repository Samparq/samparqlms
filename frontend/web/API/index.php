...
$Url='http://hrd.qdegrees.com/API/Upload_Files/';
/**************************************** For LOg In ***************************************************/
if(isset($_POST['update_phone_number']))
{
	...
	$user_id=$_POST['id'];
        ...
        $Update=mysql_query("update user set mobile_no='".$mobile_no."' where id='".$user_id."'") or die(mysql_error());
		...
		{ 
			...
			$report = array();
                        $report["success"] = 1;
                        $report["message"] = "mobile number updated successfully";
		...
		else
		{
				$report = array();
				$report["success"] = 0;
				$report["message"] = "failed to updated mobile number";
				echo json_encode($report); die();
		}

}
if(isset($_POST["getpolicy_type"])){
    $userid = $_POST["user_id"];
    $query = mysql_query("SELECT * FROM policy_type_master WHERE flag = 'ACTIVE'") or die(mysql_error());
...
if(isset($_POST["getuser_profile"])){
		...
		if(!empty($Insert_SentBox))
			{			
				...
				for($i=0;$i<sizeof($To_Array);$i++){
					$to_jisd=$To_Array[$i];
					$Select_Username_query=mysql_query("select id,app_regid,(select name from user where id='".$User_id."')as SenderName from user where user_type='".$to_jisd."' and id!='".$User_id."' and flag='ACTIVE'") or die(mysql_error());
				...
				if(!empty($Insert_SentBox))
						{			
							if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
								...
								$uploads_dir = $_SERVER["DOCUMENT_ROOT"]."/API/Upload_Files/";
					...
					/*if (is_uploaded_file($_FILES['Filedata1']['tmp_name'])) {
						$uploads_dir = './';
							...
							if (is_uploaded_file($_FILES['Filedata1']['tmp_name'])) {
									//$uploads_dir = '/Upload_Files';
										$uploads_dir = $_SERVER["DOCUMENT_ROOT"]."/API/Upload_Files/";
								//	$uploads_dir = $_SERVER["DOCUMENT_ROOT"]."/yiiapi/frontend/web/API/Upload_Files/";
									$tmp_name = $_FILES['Filedata1']['tmp_name'];
									$pic_name = $_FILES['Filedata1']['name'];
									$fname=$uploads_dir.$pic_name;
									move_uploaded_file($tmp_name, $fname);
...
if(isset($_POST['upload_policies'])){
	...
	$policy_type_id = $_POST['policy_type'];
	$policy_type_name = $_POST['policy_type_name'];
	...
	if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
		...
		orignal_filename='".$Orignal_File1_Name."',ext='".$File1_Extention."', policy_type = '".$policy_type_id."', file_size='".$FileSize."',created_by='".$user_id."', description='".$DescriptionStr."',title='".$TitleStr."'") or die(mysql_error());
		...
		$getRegId=mysql_query("select app_regid from user where flag='ACTIVE'")or die(mysql_error());
...
if(isset($_POST['get_policy_list'])){
	...
	$GetPolicyList= mysql_query("select * from policies where flag=1 AND policy_type = $typeid order by id DESC")or die(mysql_error());
...
if(isset($_POST['get_chat_contact_list'])){
	...
	$GetContact_List=mysql_query("select id,name,image_path,image_name,(select type from profile where id=u.user_type)as department from user u where id !='".$user_id."' and flag='ACTIVE' order by name")or die(mysql_error());
