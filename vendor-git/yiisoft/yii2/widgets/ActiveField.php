...
class ActiveField extends Component
{
    ...
    public function textInput($options = [])
    {
        ...
        $this->addAriaAttributes($options);
        $this->adjustLabelFor($options);
        $placeholder = $this->model->attributeLabels();
        if(empty($options['placeholder']) && !empty($placeholder)){
            $cleanLabel = strtolower(preg_replace('/[^a-z _]/i','',$this->attribute));
            @$options['placeholder'] = $placeholder[$cleanLabel];
        } else {
            $options['placeholder'] = ucwords(strtolower(preg_replace('/[^a-z ]/i','',$this->model->getAttributeLabel($this->attribute))));
        }
